...
import { MinistryTeam, VolunteerApplication } from '@/lib/types';
...
const ManageServe: React.FC = () => {
    ...
    const saveTeamMutation = useMutation({
        mutationFn: async (teamData: Partial<MinistryTeam>) => {
            ...
            const url = editingTeam ? `/api/ministry-teams/${editingTeam.id}` : '/api/ministry-teams';
                        ...
                        <div>
                            ...
                            <div className="space-y-4">
                                {teams.length > 0 ? teams.map(team => (
                                    <div key={team.id} className="admin-card p-4 rounded-xl flex items-center justify-between">
                                        ...
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <button type="button" onClick={() => handleDeleteTeam(team.id!)} className="text-red-500 hover:text-red-400 p-2 rounded-md hover:bg-red-500/10" title="Delete team"><ICONS.Trash2 className="w-5 h-5"/></button>
                                        </div>
                                    </div>
                                ...
                                <div key={app.id} className="admin-card p-4 rounded-xl">
                                    ...
                                    {app.status === 'Pending' && (
                                         <div className="mt-4 pt-4 border-t border-brand-muted flex justify-end space-x-2">
                                            <Button size="sm" variant="secondary" onClick={() => updateAppStatusMutation.mutate({ id: app.id!, status: 'Rejected' })}>Reject</Button>
                                            <Button size="sm" variant="primary" onClick={() => updateAppStatusMutation.mutate({ id: app.id!, status: 'Approved' })}>Approve</Button>
                                         </div>
                                    )}
                                </div>
