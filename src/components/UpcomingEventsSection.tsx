  ...
  return (
    ...
    >
      ...
      <div className="p-6">
        ...
        <h4 id={`event-title-${event._id}`} className="text-xl font-bold font-serif mt-2 line-clamp-2">{event.title}</h4>
        <p className="text-brand-text-dark mt-2 text-sm line-clamp-3" aria-describedby={`event-details-${event._id}`}>
          {truncateDescription(event.description, getDescriptionLength())}
        </p>
        
        {/* Event Details */}
        <div id={`event-details-${event._id}`} className="mt-4 space-y-2 text-sm" role="group" aria-label="Event details">
