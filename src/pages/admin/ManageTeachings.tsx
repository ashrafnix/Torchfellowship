...
const ManageTeachings: React.FC = () => {
    ...
    const saveMutation = useMutation({
        mutationFn: (teaching: Partial<Teaching>) => {
            ...
            const url = editingTeaching ? `/api/teachings/${editingTeaching._id}` : '/api/teachings';
            ...
            {isLoading ? (
                ...
                <div className="space-y-4">
                    {teachings.length > 0 ? teachings.map(teaching => (
                        <div key={teaching._id} className="bg-brand-dark p-4 rounded-md flex items-center justify-between border border-brand-muted hover:border-brand-gold/30 transition-colors">
                            <div>
