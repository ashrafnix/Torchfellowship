            ...
            {isLoading ? <div className="flex justify-center"><Spinner /></div> : (
                <div className="space-y-4">
                    {filteredTestimonies.length > 0 ? filteredTestimonies.map(t => (
                        <div key={t.id} className="admin-card p-4 rounded-xl">
                            ...
                            <div className="mt-4 pt-4 border-t border-brand-muted flex justify-end space-x-2">
                                {!t.is_approved && (
                                    <Button size="sm" variant="primary" onClick={() => approveMutation.mutate(t.id!)} isLoading={approveMutation.isPending}>
                                        Approve
                                    </Button>
                                )}
                                <Button size="sm" variant="danger" onClick={() => handleDelete(t.id!)} isLoading={deleteMutation.isPending}>Delete</Button>
                            </div>
                        </div>
